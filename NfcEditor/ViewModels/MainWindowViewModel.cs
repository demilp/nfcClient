...
namespace NfcEditor.ViewModels
{
    class MainWindowViewModel : INotifyPropertyChanged
    {
        ...
        public MainWindowViewModel()
        {
            ...
            id = ConfigurationManager.AppSettings["nfcId"];
            client = new BypassClient(ConfigurationManager.AppSettings["ip"], int.Parse(ConfigurationManager.AppSettings["port"]), ConfigurationManager.AppSettings["delimiter"], "nfc" + id, "tool");
        ...
        private void MessageReceivedHandler(ProximityDevice sender, ProximityMessage message)
        {
            try
            {
                using (var reader = DataReader.FromBuffer(message.Data))
                {
                    ...
                    Url = receivedString;
                    client.SendData(receivedString, "nfcLisntener" + id);
                }
